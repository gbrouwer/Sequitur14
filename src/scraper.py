...
def scrape_arxiv(config):
    job = JobManager(config, mode="scrape")
    ...
    time_ranges = list(_generate_time_ranges(start_year, end_year, sampling_freq))

    for start, end in tqdm(time_ranges, desc="Scraping arXiv windows"):
